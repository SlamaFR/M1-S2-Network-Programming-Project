...
public record FusionChangeLeader(InetSocketAddress address) implements Packet {
    ...
    public ByteBuffer toByteBuffer() {
        ...
        buffer.put(OpCode.FUSION_CHANGE_LEADER.getCode());
    ...
    public static Reader<FusionChangeLeader> getReader() {
        return new AbstractPacketReader<>() {

            private byte[] bytes;
            private int port;

            private final MultiPartReader<FusionChangeLeader> reader = new MultiPartReader<>(List.of(
                    MultiPartReader.getBytes(b -> bytes = b),
                    MultiPartReader.getInt(i -> port = i)
            ), () -> {
                try {
                    return new FusionChangeLeader(new InetSocketAddress(InetAddress.getByAddress(bytes), port));
            ...
            @Override
            MultiPartReader<FusionChangeLeader> reader() {
                return reader;
            }
        };
    }
